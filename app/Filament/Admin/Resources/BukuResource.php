...
use App\Filament\Admin\Resources\BukuResource\Pages\ListQRCode;
use App\Filament\Admin\Resources\BukuResource\Pages\ViewBuku;
use App\Models\Buku;
use Filament\Forms;
...
class BukuResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form->schema([
            ...
            Select::make('tahun_terbit')->label('Tahun Terbit')
            ->options(array_combine(range(date('Y'), 1980), range(date('Y'), 1980)))
            ->searchable()
            ->nullable()
            // 🔹 Saat membuka form Edit → hanya ambil tahun
            ->afterStateHydrated(function ($set, $state) {
            if ($state) {
            $set('tahun_terbit', date('Y', strtotime($state)));
             }})
            // 🔹 Saat disimpan → ubah ke format YYYY-01-01 biar valid DATE
            ->dehydrateStateUsing(function ($state) {
            return $state ? $state . '-01-01' : null;
            }),
            ...
            Select::make('id_kategori')->label('Kategori')->options(\App\Models\Kategori::pluck('name', 'id')->toArray())->searchable()->required(),
            Select::make('id_rak')->label('Rak')->options(\App\Models\Rak::pluck('name', 'id')->toArray())->searchable()->required(),
            Select::make('id_penerbit')->label('Penerbit')->options(\App\Models\Penerbit::pluck('name', 'id')->toArray())->searchable()->required(),
        ]);
    }
